...
SELECT @PrintMessage = 'Check if subscriber exists. Current time: '+CONVERT(varchar(20), getdate(), 121)+', Elapsed time(ms): ' + CAST(DATEDIFF(millisecond, @PreviousTimestamp, GETDATE()) AS VARCHAR(100))
, @PreviousTimestamp = GETDATE()	
RAISERROR(@PrintMessage, 0, 0) WITH NOWAIT	

IF DB_ID(@destination_db) IS NOT NULL
BEGIN
	DECLARE @tbl TABLE (col VARCHAR(10))
		,@exec_query nvarchar(1000)= 'select 1 from ' + @publisher_db + '.sys.tables where name = ''MSsubscription_properties'' and is_ms_shipped = 1' 
	INSERT INTO @tbl(col)
	EXEC (@exec_query)

	IF EXISTS(SELECT * FROM @tbl)
	BEGIN
		DECLARE @Query nvarchar(4000)= 'USE ' +@destination_db +
		...
		SELECT @PrintMessage = ''Dropping subscription. Current time: ''+CONVERT(varchar(20), getdate(), 121)+'', Elapsed time(ms): '' + CAST(DATEDIFF(millisecond, @PreviousTimestamp, GETDATE()) AS VARCHAR(100))
			, @PreviousTimestamp = GETDATE()	
		RAISERROR(@PrintMessage, 0, 0) WITH NOWAIT	
		...
		SELECT @PrintMessage = ''Dropped subscription. Current time: ''+CONVERT(varchar(20), getdate(), 121)+'', Elapsed time(ms): '' + CAST(DATEDIFF(millisecond, @PreviousTimestamp, GETDATE()) AS VARCHAR(100))
			, @PreviousTimestamp = GETDATE()	
		RAISERROR(@PrintMessage, 0, 0) WITH NOWAIT	'
