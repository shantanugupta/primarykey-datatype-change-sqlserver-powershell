...
@sql_command = N'USE '+@publisher_db+'
IF NOT EXISTS(SELECT * FROM '+@publisher_db+'.sys.tables where name = ''SS_20161228_Db_Disk_Space_Stats'')
BEGIN
CREATE TABLE SS_20161228_Db_Disk_Space_Stats
(
	dbname varchar(20), 
	[FileName] varchar(30), 
...
INSERT INTO SS_20161228_Db_Disk_Space_Stats
(
dbname,
[FileName],
CurrentSize,
...
SELECT 
	'''+@publisher_db+''' AS DbName, 
	name AS FileName, 
	size/128.0 AS CurrentSizeMB,
	GETDATE()
FROM '+@publisher_db+'.sys.database_files
UNION
SELECT 
	''tempdb'' AS DbName, 
	name AS FileName, 
...
EXEC msdb.dbo.sp_add_jobserver @job_name=@job_name, @server_name = @job_server_publisher
